...
@Preview(showBackground = true)
@Composable
fun CustomLazyRowExample() {
    val itemsList = List(20) { "Item #$it" }
    LazyRow(
        contentPadding = PaddingValues(horizontal = 16.dp), // Отступы вокруг списка
        horizontalArrangement = Arrangement.spacedBy(10.dp) // Расстояние между элементами
    ) {
        items(itemsList) { item ->
            Text(
                text = item,
